...
async def run_batch_mode(client, payloads, task_ids, result_path, max_concurrent, timeout):
    ...
    return succeeded, failed, reward_sum
...
async def run_individual_mode(client, payloads, task_ids, result_path, timeout):
    ...
    for idx, exc in submit_failures:
        ...
        logger.warning(f"[{failed}/{len(payloads)}] Task {task_ids[idx]} submission failed: {exc}")
        append_result_to_file(result_path, record)
    ...
    for i, result in enumerate(results):
        ...
        if isinstance(result, BaseException):
            ...
            logger.warning(
                f"[{succeeded + failed}/{len(payloads)}] Task {task_ids[idx]} failed "
            ...
            reward_sum += reward_val
            logger.info(
                f"[{succeeded + failed}/{len(payloads)}] Task {task_ids[idx]} completed "
                f"in {future.elapsed():.1f}s - reward: {reward_val}"
            )
...
async def main():
    ...
    if args.mode == "batch":
        ...
        succeeded, failed, reward_sum = await run_batch_mode(
        ...
        succeeded, failed, reward_sum = await run_individual_mode(client, payloads, task_ids, result_path, args.timeout)
