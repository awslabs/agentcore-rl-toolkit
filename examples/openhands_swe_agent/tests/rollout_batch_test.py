#!/usr/bin/env python
"""Unit tests for the eval harness's own seams: late-bound LLM config, and how a
failed rollout is recorded.

The two pieces under test are the parts that are the harness's
rather than the shared bounded run's:

* :class:`LateBoundLlmSession`, which defers building the perishable
  ``task["llm"]`` until inside the bounded run. Every rollout coroutine is created
  up front, so anything prepared at dispatch rots while its rollout waits in the
  queue -- a Bedrock bearer token minted then is expired long before the container
  it was minted for exists.
* :func:`run_one`'s bookkeeping, driven end to end against a fake session. The
  regression that matters is a rollout whose dump reports failure: eval used to
  record it ``aborted=False`` with whatever ``reward`` the dump carried, which
  inflated ``rollouts_ok`` and polluted the report's numeric stats.

No AWS: the session store, the S3 upload and the rollout session are all
replaced.
"""
...
# Failed without saying why: the container returned a dump but recorded no trace.
...
class LateBoundLlmSessionTest(IsolatedAsyncioTestCase):
    ...
    async def test_it_satisfies_the_session_interface(self):
        # It is handed to run_rollout_with_bounds in place of a real
        # session, so it has to be one.
    ...
    async def test_a_none_config_leaves_the_key_absent(self):
        # NullEndpoint (oracle and other no-LLM baselines) has no inference. The
        # key must stay unset rather than be set to None, so a session that did
        # read it fails loudly instead of talking to a stray model.
    ...
    async def test_the_rest_of_the_lifecycle_is_delegated_untouched(self):
        # The wrapped session keeps owning the container.
    ...
    async def test_the_null_endpoints_own_config_is_none(self):
        # The contract the wrapper's None branch exists for.
...
class LocalBoundsTest(unittest.TestCase):
    ...
    def test_rollout_concurrency_defaults_to_non_binding(self):
        # None means every container that exists may run, i.e. the rollout slot
...
def eval_config(**overrides):
    kwargs = dict(
        ...
        timeout=30,
        # Never written to: these tests drive `run_one` and `local_bounds`, not the
        # `run_eval` that reports.
        report_dir="unused",
...
class RunOneTest(IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        # the session store keeps its state in memory here: persistence is
        # persistent_dict's own test's subject, not this one's
    ...
    async def test_the_dumps_metrics_are_flattened_onto_the_row(self):
        ...
        # without knowing any metric's name.
    ...
    async def test_a_failed_dump_is_recorded_as_aborted(self):
        # The regression: a dump reporting failure used to be recorded aborted=False,
        # so it counted towards rollouts_ok and its fields entered the ok stats.
        row, _ = await self.run_one(FAILED)
        self.assertIs(row["aborted"], True)

    async def test_no_reward_is_read_off_a_failed_dump(self):
        # A null reward keeps the attempt in the pass@k denominator without counting
        # as a pass; reading the dump's own reward would have reported whatever the
        # crashed harness happened to leave there.
        row, _ = await self.run_one(FAILED)
        self.assertIsNone(row["reward"])
        self.assertIs(row["resolved"], False)

    async def test_a_failed_attempts_metrics_survive(self):
        # Aborting must not throw away the evidence: the failed dump's own numbers
        # are how a crash gets diagnosed.
        row, _ = await self.run_one(FAILED)
    ...
    async def test_the_containers_own_trace_is_recorded_unwrapped(self):
        # The failure happened in the container, so the only stack trace worth
        # keeping is that one. Recording it verbatim -- rather than raising here and
        # storing a local traceback that only points back at the harness.
        _, dumped = await self.run_one(FAILED)
        self.assertEqual(dumped["exception"], "Traceback: agent crashed")

    async def test_a_silent_failure_is_aborted_with_a_reason(self):
        # No exception from the container either. It still must not be counted as a
        # rollout that worked, and the recorded exception must not be null while
        # aborted is true, or the failure is uncategorisable in the report.
