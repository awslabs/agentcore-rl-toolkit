#!/bin/bash
# Megatron LoRA variant of fsdp_lora_sync_grpo.sh for Qwen3-4B on 8 GPUs.
# Setup: uv sync --extra verl --group verl-megatron (Python 3.12).
# Reuses agentcore_agent.yaml, including its 2048-token per-call generation limit.
# Recompute transformer layers to fit 16K training on A100 40GB.
set -euo pipefail
...
GSM8K_DIR=${GSM8K_DIR:-$SCRIPT_DIR/gsm8k}
MODEL_PATH=${MODEL_PATH:-Qwen/Qwen3-4B-Instruct-2507}
PROJECT_NAME=${PROJECT_NAME:-agentcore_grpo}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-gsm8k_qwen3_4b_megatron_lora16}
...
python3 -m verl.trainer.main_ppo \
    ...
    data.train_files="['$GSM8K_DIR/gsm8k_agent_train.parquet']" \
    data.val_files="['$GSM8K_DIR/gsm8k_agent_test.parquet']" \
